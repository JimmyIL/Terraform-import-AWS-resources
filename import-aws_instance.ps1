...
"@
  ...
  $ids = (Get-EC2Instance -region $region).Instances.InstanceId | Select-Object -Unique 
  
  #foreach found do this
  foreach ($number in $ids) { 
    $resourcetags = Get-EC2Tag -region $region | Where-Object -Property ResourceId -Match $number 
    $resourceid = $resourcetags.Value -split "/n", ""  
